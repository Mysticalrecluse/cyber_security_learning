...
- 它定义了程序员或编译器如何与处理器交互，包括指令格式、数据类型、寄存器、寻址方式和中断机制等。

- 你可以把 ISA 理解为**“编写汇编语言时你能看见和使用的全部内容的集合”**。
...
| 组成                               | 内容描述                        | 举例（x86）                         |
| ---------------------------------- | ------------------------------- | ----------------------------------- |
| ✅ **指令集（Instruction Set）**    | 所有 CPU 支持的机器指令         | `MOV`, `ADD`, `JMP`, `INT`, `MUL`   |
| ✅ **寄存器（Register Set）**       | 可以访问和操作的 CPU 内部寄存器 | `EAX`, `ESP`, `RIP`, `CR3`          |
| ✅ **寻址方式（Addressing Modes）** | 如何获取操作数的地址            | 立即数、基址+偏移、寄存器间接       |
| ✅ **数据类型（Data Types）**       | 支持的数据宽度和类型            | `byte`, `word`, `dword`, `float`    |
| ✅ **中断与异常机制**               | 支持的系统调用/中断方式         | `int 0x80`, `syscall`, `#GP`, `#PF` |
| ✅ **指令格式**                     | 指令在内存中的编码格式          | `opcode + modrm + imm`              |
...
| 层次     | 表示                | 描述                                               |
| -------- | ------------------- | -------------------------------------------------- |
| ISA      | `000000B8 01000000` | 机器码（CPU 读的）                                 |
| 汇编     | `mov eax, 1`        | 汇编程序员写的                                     |
| 高级语言 | `x = 1;`            | C/Go/Python 写的，由编译器翻译成汇编再汇编成机器码 |
...
- **机器码就是使用这个规则“写出来”的一篇文章**



##### 通过示例说明ISA，机器码，汇编之间的关系

以 x86 为例：

| 汇编指令     | 对应机器码       | ISA 层描述                                                   |
| ------------ | ---------------- | ------------------------------------------------------------ |
| `mov eax, 1` | `B8 01 00 00 00` | 指令名称是 `MOV`，目的寄存器是 `EAX`，立即数为 1，编码方式：opcode `B8` 表示 `MOV reg32, imm32` |
...
```ABAP
如果机器码是一门 CPU 能听懂的“语言”，而 ISA 就是这门语言的“语法”。
```
...
![image-20250531155529132](../markdown_img/image-20250531155529132.png)



##### 指令字结构分类

- **定长指令字结构**：所有指令的长度都一样（单字长指令，半字长指令，双字长指令）

- **变长指令字结构**：根据功能长度变化

  > 现代CPU X86-64架构，字长是64bit，表示CPU一次处理数据的位数。



##### 零地址指令

![image-20250531160449750](../markdown_img/image-20250531160449750.png)
...
| 类型                 | 描述                                 | 示例                                           | 地址字段存在？ | 地址是否参与数据操作？ |
| -------------------- | ------------------------------------ | ---------------------------------------------- | -------------- | ---------------------- |
| **① 完全无地址字段** | 操作与数据无关，仅执行流程或状态控制 | `NOP`, `HALT`, `RET`, `WAIT`, `EI`（使能中断） | ❌ 没有         | ❌ 不参与               |
| **② 隐式默认地址**   | 操作依赖固定位置（如栈顶）进行计算   | `ADD`, `MUL`, `DIV`（栈式架构）                | ✅ 有隐含地址   | ✅ 参与（隐式）         |



##### 一地址指令

​                                                                             <img src="../markdown_img/image-20250531161222884.png" alt="image-20250531161222884"  /> 



- 在A1地址读取数据，OP操作完成后，再把结果存回A1

  > 比如：自增自减指令

- 隐含约定的地址的双操作数指令，一个数据来自A1，另一个数据来自ACC（累加器）

- 如果指令长度为32bit，操作码占8bit，地址占24bit，则指令操作数据的寻址范围为`2^24 = 16M`

  > X86-64架构，         指令长度1~15字节，   不定长
  >
  > RISC-V/ARM架构，指令长度固定4字节，   定长



##### 二地址指令

![image-20250531162122457](../markdown_img/image-20250531162122457.png)



- 读取A1和A2地址数据，OP操作完成后，再把结果存回到 A1
- 如果指令长度为32位，操作码占8位，两个地址各占12位，则指令操作数据的寻址范围为`2^12 = 4K`



##### 三地址指令

![image-20250531162719720](../markdown_img/image-20250531162719720.png)

- 读取A1和A2地址数据，OP操作完成后，再把结果存回到A3
- 如果指令长度为32位，操作码占8位，两个地址各占8位，则指令操作数据的寻址范围为`2^8 = 256`
- 如果指令和所有数据都在主存中，三地址指令执行需要访问主存4次



##### 四地址指令

![image-20250531163149637](../markdown_img/image-20250531163149637.png)

- 读取A1和A2的地址数据，OP操作完成后，再把结果存回到A3
- A4存放的下一条要执行的指令地址
- 如果指令长度为32位，操作码占8位，四个地址各占6位，则指令操作数据的寻址范围为`2^6 = 64`
- 执行完本指令后，马上会执行A4指定的地址内的指令
...
![image-20250531164615366](../markdown_img/image-20250531164615366.png)



**解析第一行ARM指令集**

- Cond[31:28]：条件码，控制该指令是否在当前条件下执行
- Type/Format：[27:20] 等若干比特组合，确定指令类别（数据处理、加载存储、分支、协处理器等）
- Opcode[24:21]：指定具体操作类型，如 `0100` 表示 ADD
- S 位：是否更新标志寄存器（CPSR）
- Rn：第一个源寄存器（如加法的左操作数）
- Rd：结果写入寄存器
- Operand2：第二操作数，可以是立即数或移位寄存器
...
- **数据传输**
  - `LDR/STR`：用于加载和存储操作，可以将数据从内存传送到寄存器，或者从寄存器传送到内存
  - `MOV`：将一个寄存器的值移动到另一个寄存器（**MOV不能从主存拿数据，只能从寄存器**）
...
- **移位操作**
  ...
  - `ROR`：循环右移
- **转移操作**
  - `B/BL`：无条件跳转，BL还会保存返回地址
  - `BX`：通过寄存器值跳转
  - 条件转移指令，如BEQ（等于时跳转），BNE（不等于时跳转）等
- **输入输出操作**
  - ARMv7指令集本身不直接提供输入输出指令，这些操作通常通过操作系统或特定硬件接口来实现。但是，可以使用LDR和STR指令与内存映射的I/O接口进行数据交换
...
```basic
在 ARM 架构中，是否更新标志寄存器（CPSR，Current Program Status Register），确实要看指令中的 S 位（Set condition flags）是否置位。
ARM 中的大多数数据处理指令（如 ADD, SUB, MOV 等），只有当 S 位为 1 时，才会影响 CPSR 中的标志位（N、Z、C、V）。
        
示例：
        ADD R0, R1, R2      ; 不更新 CPSR
        ADDS R0, R1, R2     ; ADD + S：更新 CPSR
        
示例2：（比较类指令隐含 S）
        CMP R1, R2          ; 实际执行的是 SUBS R1, R2 且丢弃结果
```
...
![image-20250531180323615](../markdown_img/image-20250531180323615.png)

**条件码（Cond）**

表示在什么情况下，该条指令才会执行（所有指令能否执行都受条件码影响）

> 大部分指令条件码（高4位）都是1110，即无条件执行
...
- **数据传送**
  - **MOV**：将数据从一个地方移动到另一个地方，例如从寄存器到寄存器，从内存到寄存器，或反之。
  - **PUSH/POP**：用于堆栈操作，将数据推送到堆栈或从堆栈弹出
...
- **转移操作**
  - **JMP**：无条件跳转
  - 条件跳转指令，如 JE（跳转如果等于），JNE（跳转如果不等于）等。
  - **CALL**：调用子程序
  - **RET**：从子程序返回
- 输入输出操作
  - **IN**：从端口读取数据到寄存器
  - **OUT**：将数据从寄存器写入端口
  - 这些指令用于与外部设备进行数据交换，通常在更低级别的系统编程中使用
...
- **统一编译**：I/O与内存统一编址，如ARM，访问外设如同访问内存

  > ARM CPU的CPU文档（芯片datasheet）里，有清除的描述，在指定区域内，哪个地址用来给哪个硬件寄存器使用，可以通过访问指定地址，来访问制定寄存器

- **独立编制**：I/O与内存分开编址，如X86，专门的CPU指令

  > X86中，有专门的指令用来访问外设，IN/OUT，比如OUT输出到哪个地址，后面跟上对应的外设地址。
...
![image-20250531212824617](../markdown_img/image-20250531212824617.png)
...
> 上述有10中寻址，因此寻址特征最少需要4bit



关于寻址特征的解读

| 寻址特征字段（2~3 位） | 含义                               | 寻址方式                |
| ---------------------- | ---------------------------------- | ----------------------- |
| `00`                   | 操作数就在地址字段里               | **立即寻址**            |
| `01`                   | 地址字段给出的是内存地址           | **直接寻址**            |
| `10`                   | 地址字段中是一个指针，指向真正地址 | **间接寻址**            |
| `11`                   | 地址字段是偏移，需要和寄存器相加   | **变址寻址 / 基址寻址** |

这些值是 CPU 取指时用于判断**如何解析“形式地址”字段**的依据。



##### 隐含寻址

![image-20250531212945766](../markdown_img/image-20250531212945766.png)

**典型案例1：（隐含操作数是 ACC）**

```assembly
CLR    ; Clear ACC 累加器，操作数隐含为 ACC
INC    ; 对 ACC 做加一
```

这些指令中 **没有操作数字段**

CPU 知道要操作的是固定寄存器，如 `ACC`（累加器）



**典型案例2：隐含地址在“栈顶”的例子（0 地址机器）**

```assembly
POP    ; 隐含操作数为 栈顶
ADD    ; 隐含两个操作数都来自栈顶，执行 SP[-1] + SP[-2]
```
...
![image-20250531214010781](../markdown_img/image-20250531214010781.png)
...
```assembly
mov R0, #10   ; 10就是立即数，可以直接赋给寄存器R0使用
```
...
![image-20250531214445873](../markdown_img/image-20250531214445873.png)
...
```assembly
;X86
MOV AX, [0x1234]  ; 直接从内存地址 0x1234 读取值，放入 AX

; ARM
LDR R0, =0x20000000   ; 实际效果是从地址 0x20000000 读取值 → 放入 R0（PC 相对寻址 + 文字池）
;等价于
LDR R0, [PC, #offset]  ; offset 是跳转到文字池的距离
```
...
; X86
; 间接寻址
mov eax, [ebx]       ; 读取 ebx 所指地址处的数据到 eax
...
![image-20250531221526092](../markdown_img/image-20250531221526092.png)
...
; 变址寻址
EA = (IX) + A ; 操作码 + 寻址特征 + 数据首地址 + 变址寄存器，变址寄存器IX中的值会自行变化
```
...
压栈、出栈操作，不指定操作内容地址，默认使用**SP寄存器**的地址



### 指令执行

<img src="../markdown_img/image-20250601002733480.png" alt="image-20250601002733480" style="zoom:150%;" />
...
微程序控制器同硬布线控制器相比较，具有规整性，灵活性，可维护性等一系列优点，因而在计算机设计中逐渐取代了早期采用的硬布线控制器，并已广泛地应用。在计算机系统中，微程序设计技术是利用软件方法来设计硬件的一门技术。

微服务控制的基本思想，就是仿照通常的解题程序的方法，把操作控制信号编成所谓的“微指令”，存放在一个只读存储器里。当机器运行时，一条又一条地读出这些微指令，从而产生全机所需要的各种操作控制信号，使相应部件执行所规定的操作。
...
- CPU立即强行中止现行程序的运行，启动相应的程序来处理这些"事件"
- 处理完后，OS恢复原来的程序运行或终止原来的程序
- 这些 “事件” 被称为 “中断” 或 “异常”

![image-20250601004739357](../markdown_img/image-20250601004739357.png)



#### 中断的类型

- 内部中断（异常）：由CPU内部异常引起的事件
  - 软件故障：除0，非法地址，缺页
  - 硬件故障：电源断电，线路故障
  - 自陷（软中断）：通过系统调用调用内核函数
- 外部中断：IO设备有事件需要处理时向CPU发送中断请求
  - 键盘鼠标事件
  - 硬盘事件
...
![image-20250601005627505](../markdown_img/image-20250601005627505.png)
...
流水线的本质就是：CPU 内部的多个功能部件（组件）同时处理不同指令的不同阶段，从而实现“阶段级别”的并发执行。

![image-20250601010312643](../markdown_img/image-20250601010312643.png)
...
- RISC指令更适合流水线的实现
- Intel在微体系结构层把复杂的CISC指令分解成RISC风格的指令，以方便流水线的实现
- **5阶段流水线**
  - **Ifetch**：取指令，PC+4
  - **Reg/Dec**：读寄存器，指令译码
  - **Exec**：ALU运算
...
- 每条指令统一填充成五阶段，填充的阶段用空（NOOP）操作，使下一条指令的最后阶段总是比上一条指令晚一个周期

  > 避免资源冲突

![image-20250601011051232](../markdown_img/image-20250601011051232.png)



#### 五段流水线数据通路

![image-20250601011938081](../markdown_img/image-20250601011938081.png)



#### 流水线深度

- ARM 或 Intel i7 CPU流水线深度是14
- 流水线并不是越深越好，要折中选择一个合理的流水线深度
  - 每增加一段就需要一个段寄存器，写寄存器需要时间
  - 主频的额提升和晶体管数量的增加都使CPU的功耗变大
...
- 指令流水线执行过程中，相邻或相近的指令之间因存在某种依赖性，使得指令的执行可能受到影响，称为流水线冒险
...
- 解决方案：
  - 插入空指令
  - 指令存储器和数据存储器分开
  - 一级高速缓存分指令cache和数据cache（现代CPU解决方案）

![image-20250601012559880](../markdown_img/image-20250601012559880.png)

##### 数据冒险

- 数据依赖性：需要等待之前的指令完成数据的读写
- 解决方案：
  - 编译器插入控制流
  - 硬件停顿
  - 前向传递（现代CPU解决方案）

![image-20250601012907004](../markdown_img/image-20250601012907004.png)
...
- 解决方案：**分支预测**
  - 静态分支预测：假设不跳转，50%概率
  - 动态分支预测：根据历史跳转情况预测（现代CPU解决方案，成功率90%以上）

![image-20250601013207215](../markdown_img/image-20250601013207215.png)
...
- 通常具有两条或两条以上并行工作的流水线结构称为超标量/多发射，与之相对之前的流水线则称为“标量流水线”
- CPI的倒数，又称为IPC（Instruction Per Clock），也就是一个时钟周期里面能够执行的指令数，代表了CPU的吞吐量
- 多发射/超标量可使IPC大于1

![image-20250601164735012](D:\git_repository\cyber_security_learning\markdown_img\image-20250601164735012.png)
...
- 用于解决缓存缺失导致的流水线停顿
  ...
  > 因为如果缓存缺失，CPU就只能访问主存，而CPU的速度是主存的120倍，此时就只能等待加载主存数据，在这个等待过程中，则可以切换另一个线程执行其他程序，而不是CPU空转等待，提升CPU的利用率

- 每个线程有自己的寄存器，互不影响，同时只能执行一个线程，但是共用译码器和ALU，因此性能比单核CPU提升20%~30%

![image-20250601165547436](../markdown_img/image-20250601165547436.png)
...
- **多核**：一个CPU芯片内多个处理单元，每个处理单元称为一个核，多个核共享L3缓存

![image-20250601165842825](../markdown_img/image-20250601165842825.png)
...
![image-20250601165955101](../markdown_img/image-20250601165955101.png)
...
- 一条指令并行处理多个数据的运算，用来高效的执行向量运算，视频/图像处理
- SIMD实现：
  - x86 SSE/AVX多媒体扩展指令
...
- **输入输出设备**：磁盘，光盘，网卡
- **外设的特点**
  - cpu和主存是电子设备，其他外部设备可能是光学，磁性，机械设备
  - 不同设备的信息类型和格式不同
  - 输入输出设备速度比内存和cpu慢很多
...
<img src="../markdown_img/image-20250601171136923.png" alt="image-20250601171136923" style="zoom:150%;" />



每个外设都有对应的IO控制器，比如：键盘有键盘控制器，显示器有显示器控制器......

在控制器，内存跟CPU之间通过总线来相连（每个外设都有对应的 IO 控制器，这些控制器通过主板芯片组与 CPU 通信”）



#### **IO控制器的作用**

- 信息格式转换，把字符，图形，电压等这些格式转换成CPU能够识别的二进制信息，同时把CPU输出的二进制信息转换成这些设备所规定的格式
- 信息的缓存：因为外设的通信速度比CPU处理速度慢很多，因此这些设备的信息先放在缓存里面，再通知CPU去取，CPU写出的信息也先放到缓存里面，然后IO控制再去慢慢的写入到对应的外设
- CPU通过IO控制器提供的各种寄存器来跟它进行通信，包括控制寄存器，状态寄存器跟数据寄存器
  - CPU通过控制寄存器向外设发送读写的指令
  - 通过状态寄存器去查询外设的状态，是准备就绪，还是工作中
  - 数据寄存器用来存放双方的数据
...
- IO端口和存储器统一编址
  - 存储器映射的IO方式（Memory Mapped I/O）
  - I/O端口和存储器使用一套地址空间，地址不能重复
  - ARM、MIPS、RISC-V等体系结构采用该方式

![image-20250601175206061](../markdown_img/image-20250601175206061.png)
...
- I/O端口和存储器独立编址
  - 需要专门的io指令（in/out）进行操作
  - I/O操作和存储器有各自的地址空间
  - x86体系结构采用该方式

![image-20250601175622334](../markdown_img/image-20250601175622334.png)
...
- **中断方式**：键盘，鼠标
  - io设备通知cpu
- **DMA方式**（Direct Memory Access）：硬盘，网卡
  - 不通过cpu，内存和io设备直接交换数据
  - 需要专门的DMA控制器来完成数据传输
...
![image-20250601181008713](../markdown_img/image-20250601181008713.png)
...
- C语言
  - 函数（函数内包含变量定义，语句）
  - 函数外的声明和定义（变量定义，函数声明，外部变量引用声明，结构体声明）
  - 注释，预处理指令（宏定义，头文件引用）
...
int global_counter = 0;     // 变量定义（全局变量）
...
void print_area(struct Circle c);
...
void print_area(struct Circle c) {
    ...
    float area = calculate_area(c);
    printf("Circle #%d: Radius = %.2f, Area = %.2f\n", global_counter, c.radius, area);
}
...
![image-20250601204732953](../markdown_img/image-20250601204732953.png)

**预处理（cpp）**-> **编译（ccl）**-> **汇编（as）** -> **链接（ld）**

```C
-c // 预处理，编译和汇编源文件，但是不作链接
-S // 编译后即停止，不进行汇编，用来生成汇编文件
-E // 预处理后就停止，不进行编译
-o file // 指定输出文件，i是预处理后文件，.s汇编文件，.o目标文件
-Wall //打开警告信息
-g // 打开调试信息供gdb使用
-0 // 优化选项，-00不优化，-01~-03
-ldir // 指定搜索路径，用来查找头文件
...
// 预处理
gcc -E hello.c -o hello.i  // 预处理后的文件：删除注释；
                           // 展开宏定义；
                           // 执行条件编译指令（#ifdel...），只保留满足条件的部分；
                           // 把头文件内容插入到include地方
                           // 同时编译器在编译的时候，会生成一些调试信息
...
1. **地址发出（Address Phase）**

   - CPU 将要读取的数据的 **物理地址** 放到 **地址总线** 上。
   - 同时发出控制信号：**Read（RD）= 有效**，表明这是一次读取操作。

2. **地址解码（Address Decode）**

   - **内存控制器（Memory Controller）** 监听地址总线，识别哪个物理内存区域被访问。
   - 它负责将地址传给对应的 **内存模块（DRAM 芯片）**，并控制时序。

3. **数据准备（Data Access Phase）**

   - DRAM 芯片根据地址，从其内部的存储单元中读取数据，并在数据准备好后将其放入 **数据总线**。
   - 此过程中会有等待延迟，比如 DRAM 的行激活（tRCD）、列访问（CL）等。

4. **数据传输到 CPU（Data Transfer）**

   - **内存控制器** 把数据从数据总线接收，**通过总线接口或高速互联（如AMBA, QPI, UPI）传送到 CPU。**

     > **内存控制器的职责**
     >
     > 内存控制器（Memory Controller）负责：
     >
     > - 地址解码（确定访问哪个 DRAM Bank/Row/Column）
     > - 控制时序（如 tRAS、tRCD、tCL）
     > - 发出 DRAM 所需控制信号（如 RAS、CAS、WE）
     > - 管理数据总线的读写方向
     > - **最终把数据传回 CPU**
     >
     > 在这个过程中，内存控制器充当了**桥梁**：
     >  它从 DRAM 芯片读取数据，再将数据放到 CPU 可接收的接口（如前端总线、AMBA、Intel UPI 等）上
     >
     > 
     >
     > **数据写入高速缓存的控制方是 CPU 内部逻辑**
     >
     > - 高速缓存是 CPU 内部的一部分（L1/L2/L3），
     > - **内存控制器本身并不直接操作缓存**，
     > - 它只是把数据返回给 CPU，**由 CPU 的缓存控制单元（Cache Controller）决定是否写入 Cache**，以及写入哪一层 Cache。

   - 数据在传输到 CPU 前，**通常会先写入 L1/L2 高速缓存（Cache）**（如果启用了缓存机制）。

5. **CPU 使用数据（Register Write）**

   - CPU 从缓存中读取数据，并把它复制到目标 **寄存器（Register）** 中，用于指令执行。
...
![image-20250529145912370](D:\git_repository\cyber_security_learning\markdown_img\image-20250529145912370-1748594220531-1.png)

![image-20250529150048154](D:\git_repository\cyber_security_learning\markdown_img\image-20250529150048154-1748594220531-2.png)

![image-20250529150312238](D:\git_repository\cyber_security_learning\markdown_img\image-20250529150312238-1748594220532-3.png)
...
![image-20250529150507938](D:\git_repository\cyber_security_learning\markdown_img\image-20250529150507938-1748594220532-4.png)
...
![image-20250529160504716](D:\git_repository\cyber_security_learning\markdown_img\image-20250529160504716-1748594220532-5.png)
...
- **DrMOS 80A**
  ...
  >
  > ![image-20250529161324373](D:\git_repository\cyber_security_learning\markdown_img\image-20250529161324373-1748594220532-6.png)
  >
  > 上述CPU仅仅烤机几分钟的热成像，供电模组已经达到100多度，温度高就会触发主板的保护机制，降低CPU的供电频率，就会性能下降
  >
  > 因此高端主板都会给供电模块上覆盖一层超厚的金属散热片
  >
  > ![image-20250529161603252](D:\git_repository\cyber_security_learning\markdown_img\image-20250529161603252-1748594220532-7.png)
  >
  > 除了供电模块外，所有的主板都会给芯片组覆盖散热马甲进行散热
  >
  > ![image-20250529161749114](D:\git_repository\cyber_security_learning\markdown_img\image-20250529161749114-1748594220532-8.png)
  >
...
<img src="D:\git_repository\cyber_security_learning\markdown_img\image-20250529163619417-1748594220532-11.png" alt="image-20250529163619417" style="zoom:150%;" />
...
![image-20250529173903231](D:\git_repository\cyber_security_learning\markdown_img\image-20250529173903231-1748594220532-9.png)
...
![image-20250529173946179](D:\git_repository\cyber_security_learning\markdown_img\image-20250529173946179-1748594220532-10.png)
...
![image-20250529174935321](D:\git_repository\cyber_security_learning\markdown_img\image-20250529174935321-1748594220532-12.png)
...
![image-20250530013816213](D:\git_repository\cyber_security_learning\markdown_img\image-20250530013816213-1748594281036-25.png)
...
![image-20250530025419014](D:\git_repository\cyber_security_learning\markdown_img\image-20250530025419014-1748594281036-27.png)
...
![image-20250530030011657](D:\git_repository\cyber_security_learning\markdown_img\image-20250530030011657-1748594281037-30.png)



**按总线接口分类**：分为ISA总线接口网卡（已淘汰）；PCIE总线网卡；USB网卡

![image-20250530030546497](D:\git_repository\cyber_security_learning\markdown_img\image-20250530030546497-1748594281036-26.png)



**按端口类型分类**：RJ-45端口网卡；光端口网卡

![image-20250530030957650](D:\git_repository\cyber_security_learning\markdown_img\image-20250530030957650-1748594281036-28.png)
...
![image-20250530031723367](D:\git_repository\cyber_security_learning\markdown_img\image-20250530031723367-1748594281037-29.png)



基于DPU（数据处理单元）的SmartNIC可以基于ASIC，FPGA和片上系统（SOC）技术实现

![image-20250530032536089](D:\git_repository\cyber_security_learning\markdown_img\image-20250530032536089-1748594281037-31.png)

![image-20250530033200560](D:\git_repository\cyber_security_learning\markdown_img\image-20250530033200560-1748594281037-32.png)
