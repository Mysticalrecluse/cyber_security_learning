...
# 字符串的内部实现是PyUnicodeObject结构体，包含了字符串的长度，引用计数，hash值等信息
# 字符串的数据结构是Unicode编码，这意味着字符串是不定长的，不同的字符占用不同的字节数，字符串的hash值可以在创建时计算出来并且缓存起来，这样可以提高字典的查找效率
# 字符串的本质也是顺序表，可以通过索引访问，也可以通过切片访问
