...
`sync_binlog` 控制 **MySQL 写入 binlog 后，是否立刻对 binlog 执行 fsync（把 OS page cache 刷到磁盘）**

注意：binlog 的安全性必须区分两种故障
...
- commit 会触发 write() （即 **binlog_cache -> OS page cache**）commit 成功之前 → binlog 必须 write()
- sync_binlog 控制是否执行 fsync() （**OS page cache -> 磁盘**）
- write ≠ 落盘

> `sync_binlog` 决定的是最后那一步“fsync”的频率。
...
| 值    | 含义（务必按这个理解）                                | 结果                                             |
| ----- | ----------------------------------------------------- | ------------------------------------------------ |
| `0`   | **MySQL 不主动 fsync binlog**，什么时候落盘由 OS 决定 | 性能最好，断电/宕机可能丢 binlog                 |
| `1`   | **每次事务提交都 fsync binlog**                       | 最安全，开销最大                                 |
| `N>1` | **每 N 次提交 fsync 一次**（近似理解）                | 安全与性能折中，最坏可丢近 N 次提交对应的 binlog |

> 注意：`N` 是“刷盘频率”，不是“精确每 N 个事务一定刷一次”。在实际实现中会受 group commit、并发、调度影响，但风险上可以按“最坏丢 N 次附近”。
...
> **主库崩溃/断电时，binlog 可能只写在 page cache 里，没落盘，导致 binlog 丢失**。

这会带来非常现实的后果：

- **已对客户端返回成功的事务**，主库重启后 binlog 里却找不到（因为丢了）
- 从库当然也收不到这些事务
- 你做主从切换时，可能出现“明明业务说成功了，但新主库没有”的事故



### innodb_flush_log_at_trx_commit

innodb_flush_log_at_trx_commit = 0 / 1 / 2 不同的值 **差别只在“COMMIT 时，redo 到底有没有真正落到稳定存储”**。

InnoDB 写 redo 有两步动作

- **write**：把 redo 从 InnoDB 的 redo buffer 写到 OS page cache（操作系统缓存）
- **fsync**：把 OS page cache 强制刷到磁盘/持久介质（真正抗断电）



![image-20260421150111692](D:\git_repository\cyber_security_learning\markdown_img\image-20260421150111692.png)

#### =1（最强持久性）

- 每次 COMMIT 都 write + fsync
- COMMIT 返回成功时，redo（含 commit 记录）已经在 “稳定存储” 上（理论上）
- **最多丢 0 个已提交事务**（正常硬件/文件系统前提下） 

> 代价：写压力大，但现代 MySQL 有 group commit，实际常比想象中好。
...
- 每次 COMMIT 都 write
- **fsync 由后台每秒做一次**（大约 1 秒一次）
- 崩溃类型决定会不会丢：
  - **只 crash mysqld 进程**（OS 还活着）：
    - OS page cache 还在 → 通常不丢（redo 已写到 OS cache）
  - 断电/OS 崩溃
    - OS page cache 会丢 → **可能丢最近 1 秒内“已提交”的事务**



#### =0（性能优先）

- COMMIT 时 **既不保证 write，也不保证 fsync**（由后台每秒把 redo buffer 刷到 OS cache，并每秒 fsync）
- **mysqld 进程 crash**：也可能丢（因为 redo 还在 InnoDB 内存里，没来得及 write 到 OS cache）
- **断电/OS 崩溃**：同样可能丢最近 1 秒，甚至更糟（取决于时机）

| 参数值 | COMMIT 时做了什么  | mysqld 崩溃    | OS 崩溃/断电         |
| ------ | ------------------ | -------------- | -------------------- |
| 1      | write + fsync      | 基本不丢已提交 | 基本不丢已提交       |
| 2      | write（不 fsync）  | 通常不丢已提交 | 可能丢最近≈1秒       |
| 0      | 不保证 write/fsync | 可能丢最近≈1秒 | 可能丢最近≈1秒或更糟 |

> 注意：这里的 “≈1秒” 是经验描述，本质是 “**最后一次 fsync 之后到故障发生之前**” 的窗口。



### binlog_format

`binlog_format` 决定 **MySQL binlog 记录“变更”的方式**，它直接影响：

- **主从复制一致性**（最关键）
- **binlog 体积与写入开销**
- **PITR（基于 binlog 的时间点恢复）可用性与精度**
- **审计/追溯**时你能看到什么（SQL 还是行变更）

一句话定位：

> `binlog_format` 决定 binlog 里写的是 “我执行了什么 SQL” ，还是“我最终把哪些行改成了什么”。
...
> 注意：不同版本对动态修改的限制可能不同；生产以配置文件固定为主，避免运行中切换引入不一致的排查成本。
...
```sql
UPDATE t SET c = c + 1 WHERE g = 10;
```



##### **STATEMENT（SBR：Statement-Based Replication）**

**binlog 记录：SQL 语句本身**

- binlog 相当于把你执行的 SQL 写进去
- 从库 “重放 SQL” 来得到相同结果

直观理解：

> 主库说：“我执行了这条 UPDATE…”，从库也执行同一条 UPDATE。
...
**致命缺点：不确定性（non-deterministic）导致不一致**

只要 SQL 的结果依赖“当前环境/时间/随机/执行计划/数据顺序”，主从就可能不一致。

典型风险点（你一定要讲给学生）：

- `NOW()`, `UUID()`, `RAND()`
- `LIMIT` 没有 `ORDER BY`（从库可能更新不同的行）
- 依赖系统变量、时区、字符集差异
- 触发器/存储过程里有不确定函数或外部依赖
- 并行复制/不同执行路径导致“行集合不同”

> 结论：STATEMENT 复制一致性风险最高，现代生产通常不推荐。



##### **ROW（RBR：Row-Based Replication）**

binlog 记录：行的变更结果（前镜像/后镜像）**

- binlog 记录“哪些行的哪些列改成了什么”
- 从库不需要执行 SQL，只需要“按行应用更改”

直观理解：

> 主库说：“id=123 的这一行，c 从 7 变成 8”，从库照做。

**优点（生产最看重）**

- 一致性最强：不依赖 SQL 是否确定
- 对复杂 SQL、触发器、函数、并行复制更稳
- 复制延迟/不一致问题更少

**缺点**

- binlog 体积可能显著变大（尤其是大批量 UPDATE/DELETE）
- 可读性弱（审计不如 statement 直观）
- 对写入 IO、网络、从库 apply 压力更大

> 结论：大多数 OLTP 生产推荐 `ROW`，稳定性优先。



##### **MIXED（MBR：Mixed-Based Replication）**

**binlog “优先 statement”，遇到不安全语句自动切换 row**

直观理解：

> MySQL 能确定安全就写 SQL，不能确定就写行变更。
...
- 行为会“时而 SQL、时而 ROW”，排查时不稳定
- 你很难通过固定心智模型预测某条语句到底记录成什么
- 课程上不如 ROW 清晰；生产上也不如 ROW 可控

> 结论：MIXED 是折中方案，但很多团队仍选择直接 ROW 以减少不可预测性。
...
| -------------------- | ----------- | ------------------ | -------------- |
| 一致性风险           | 高          | 最低               | 中             |
| binlog 体积          | 最小        | 最大（视写入类型） | 中             |
| 可读性/审计          | 最好（SQL） | 最差（行事件）     | 不稳定         |
| 复制 apply 压力      | 中          | 高（大事务更明显） | 中             |
| 推荐程度（现代生产） | ❌ 不推荐    | ✅ 推荐             | ⚠️ 可用但不首选 |
...
![image-20260421172616810](D:\git_repository\cyber_security_learning\markdown_img\image-20260421172616810.png)
...
## 没有 Group Commit 会发生什么

原始模型：

![image-20260421155555515](D:\git_repository\cyber_security_learning\markdown_img\image-20260421155555515.png)
...
## Group Commit 是什么（核心定义）

Group Commit = 多个事务共享一次 fsync

多个事务各自的 binlog cache，会在 group commit 的 flush 阶段被依次 write 到同一个 binlog 文件对应的 OS page cache 中，然后在 sync 阶段通过一次 fsync 一起落盘。

![image-20260421161818537](D:\git_repository\cyber_security_learning\markdown_img\image-20260421161818537.png)

## MySQL 中的 Group Commit 流程（重点）

在 MySQL（开启 binlog）中：Group Commit 主要发生在 **binlog 阶段**

> 在 group commit 中，每个事务会先生成自己的 binlog cache，在 flush 阶段这些数据会被 write 到同一个 binlog 文件对应的 OS page cache 中，这个过程可能涉及多次 write，且不一定由 leader 单独完成。
>
> 在 sync 阶段，由 leader 执行一次 fsync，将这一批事务的数据统一持久化，从而实现多个事务共享一次刷盘操作。



### MySQL官方拆分为三个阶段

![image-20260421164829740](D:\git_repository\cyber_security_learning\markdown_img\image-20260421164829740.png)

MySQL 将多个事务在 flush 阶段合并写入 binlog，在 sync 阶段统一 fsync，从而减少磁盘刷盘次数。



## 影响 Group Commit 效果的关键参数

### sync_binlog（核心）

Group Commit 和 Sync_binlog 的最大区别：**（时间线）**

| 维度                 | Group Commit            | sync_binlog=N             |
| -------------------- | ----------------------- | ------------------------- |
| 作用对象             | 同一时间的一批事务      | 不同时间的多批事务        |
| 合并方式             | 横向（多个事务 → 一组） | 纵向（多组 → 一次 fsync） |
| 是否改变 commit 流程 | 是（引入队列/leader）   | 否（只影响 fsync 时机）   |
| 数据安全             | 安全（配合=1）          | 有丢数据风险              |
| 是否需要并发         | 需要                    | 不需要                    |



### binlog_group_commit_sync_delay（延迟）

等待更多事务加入

```ini
# leader 最多等 1000 微秒（1ms），看看有没有更多事务进来
...
```ini
#如果已经有 10 个事务在等，就别再等时间了，立刻 fsync
...
> Group Commit 是 MySQL 为了减少 fsync 次数而引入的一种优化机制，它通过将多个事务的 binlog 写入合并，并共享一次 fsync，从而显著提升高并发写入场景下的性能。
>
> 在实现上，MySQL 将事务提交划分为 flush、sync 和 commit 三个阶段，并在 sync 阶段进行批量刷盘。
>
> 需要注意的是，Group Commit 的效果依赖于并发度和参数配置，同时需要在吞吐和延迟之间做权衡。
